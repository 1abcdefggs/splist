...
- **🚀 [v2.1.3 Update] Why `child_process` and `process.env` were completely removed from the Core engine**
  - **Rationale**: To resolve critical security warnings (e.g., OS command injection) flagged by security scanners like Socket.dev. By eliminating OS-level terminal execution capabilities (`child_process`) and direct environment variable access (`process.env`), the core pipeline becomes significantly safer. This strict security enhancement serendipitously made the core engine even more pure, allowing it to run natively and flawlessly inside browser environments (Web UI) without relying on mocks.
